...
{
    if (full) error("꽉 찬 버퍼에 토큰을 넣으려고 합니다");
    ...
    switch (ch) {
        ...
        default: {
            if (isalpha(ch)) {
                ...
                while (cin.get(ch) && (isalpha(ch) || isdigit(ch))) var += ch;

                cin.putback(ch);

                if (var == declkey) return Token(let, var);
    ...
    while (true) {
        switch (t.kind) {
            ...
            case '%': {
                ...
                if (i2 == 0) error("Can't do modulor operation on floating-point");
    ...
    switch (t.kind) {
        case var_kind: {
            if (is_declared(t.name))
                error(t.name.c_str(),
                      "이 var_table에 이미 존재합니다. 같은 변수를 두번 선언 할 "
                      "수 없습니다!");
