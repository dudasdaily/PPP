...
PPP_EXPORT inline void simple_error(
    std::string s)  // write ``error: s'' and exit program (for non-exception terminating error handling)
{
    std::cerr << "error: " << s << '\n';
    exit(1);
}
...
PPP_EXPORT inline void error(const std::string &s, const std::string &s2) { error(s + s2); }
...
struct std::hash<PPP::Checked_string> {
    size_t operator()(const PPP::Checked_string &s) const { return hash<std::string>()(s); }
};